...
@Component({
  ...
  : {value: 'hidden',  params: {transitionParams: '400ms cubic-bezier(0.86, 0, 0.07, 1)', height: '0'}}">
    <li *ngFor="let item of data.items" class="sublevel-nav-item">
      <a class="sublevel-nav-link" *ngIf="item.items && data.items.length > 0" (click)="handleClick(item)">
        <i class="sublevel-link.icon bi bi-dot"></i>  
        <span class="sublevel-link-text" @fadeInOut *ngIf="collapsed">{{item.label}}</span>
        <i *ngIf="item.items && collapsed" class="menu-collapse-icon" [ngClass]="!item.expanded ? 'bi bi-chevron-compact-left' : 'bi bi-chevron-compact-down'"></i>
      </a>
